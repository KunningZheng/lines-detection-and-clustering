...
def cluster_3d_segments(mask_assignments, required_views=3, n_clusters=None):
    ...
    # 转换为 NetworkX 图（LPA 通常用 NetworkX 实现）
    ...
    # 使用 Label Propagation Algorithm (LPA) 进行聚类
...
def cluster_3d_segments_CW(mask_assignments, required_views=3, n_clusters=None):
    """
...
if __name__ == '__main__':
    ...
    lines3d_clusters = cluster_3d_segments_CW(all_line3d_to_mask, required_views=1)
    # 选择前100个线段最多的cluster
    largest_clusters = sorted(lines3d_clusters.values(), key=lambda cluster: len(cluster), reverse=True)[:100]
